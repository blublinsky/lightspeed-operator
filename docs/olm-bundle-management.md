...
**Note:** While Lightspeed Operator declares support for both `OwnNamespace` and `AllNamespaces` install modes in the CSV, 
it is **hardcoded to only operate in the `openshift-lightspeed` namespace** (see `utils.OLSNamespaceDefault` in `internal/controller/utils/constants.go`). 
The operator will default to this namespace unless the `WATCH_NAMESPACE` environment variable is explicitly set. 
This is because `OLSConfig` is cluster-scoped and all operand resources are created in this fixed namespace.
