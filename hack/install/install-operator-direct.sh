...
direct_deploy_skip_idms() {
	...
	if [[ -z "$operator_img" ]]; then
		operator_img=$("${jqbin}" -r '.[] | select(.name=="lightspeed-operator") | .image' "${REPO_ROOT}/related_images.json")
	fi
	if [[ -z "$operator_img" || "$operator_img" == "null" ]]; then
		echo "error: operator image not found in related_images.json; set IMG" >&2
		exit 1
	fi
